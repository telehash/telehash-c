...
bucket_t bucket_load(xht_t index, char *file)
{
  ...
  // check each value, since js0n is key,len,value,len,key,len,value,len for objects
	...
	{
    p2 = lob_new();
    lob_json(p2, p->json+p->js[i+2], p->js[i+3]);
    hn = hashname_fromjson(index, p2);
    lob_free(p2);
    if(!hn) continue;
    if(!b) b = bucket_new();
    bucket_add(b, hn);
...
void util_sendall(switch_t s, int sock)
{
  ...
  while((p = switch_sending(s)))
  {
    ...
    DEBUG_PRINTF("<<< %s packet %d %s",p->json_len?"open":"line",lob_len(p),path_json(p->out));
    path2sa(p->out, &sa);
    if(sendto(sock, lob_raw(p), lob_len(p), 0, (struct sockaddr *)&sa, sizeof(sa))==-1) printf("sendto failed\n");
    lob_free(p);
  }  
}
