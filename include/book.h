...
chapter_t chapter_free(chapter_t chap);

// accessors
uint32_t chapter_hash(chapter_t chap);
uint32_t chapter_len(chapter_t chap);
char *chapter_title(chapter_t chap);

// pages
uint16_t chapter_first(chapter_t chap);
uint16_t chapter_last(chapter_t chap);

// new lob describing it
lob_t chapter_json(chapter_t chap);

// accumulates checkhash and returns next page to load after given data (must be len%16==0)
uint16_t chapter_pager(chapter_t chap, uint16_t at, uint8_t *bin, uint32_t len);

// success of last full pager hashing, optional flag to update w/ new hash
chapter_t chapter_verify(chapter_t chap, bool update);

// given the first page, return the last page of the lob header
uint16_t chapter_head(chapter_t chap, uint8_t *first);

// returns raw index page
uint8_t *chapter_index(chapter_t chap);
